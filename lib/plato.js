...
var util = require('./util'),
  ...
  getReportCharts = require('./charts').getReportCharts,
  fileChart = require('./charts').fileChart,
  reporters = {
...
var overviewTemplate = __dirname + '/templates/overview.html',
  staticOverviewTemplate = __dirname + "/templates/static-overview.html",
  displayTemplate = __dirname + '/templates/display.html',
  fileTemplate = __dirname + '/templates/file.html',
  barChartTemplate = __dirname + '/templates/bar-chart.html',
...
exports.inspect = function (files, outputDir, options, done) {
  ...
  if (options.jshint) {
    flags["jshint"] = options.jshint;
  }

  if (options.eslint) {
    flags["jshint"] = options.eslint;
  }
    ...
    fs.mkdirp(fileOutputDir, function () {
      runReports(files, function () {
        ...
        fs.copy(assets, path.join(outputDir, 'assets'), function () {
          var overviewReport = exports.getOverviewReport(reports, options);
          updateHistoricalOverview(reportFilePrefix, overviewReport, options);
          writeReport(reportFilePrefix, overviewReport);

          overviewReport.charts = options.static ? getReportCharts(overviewReport, barChartTemplate) : {};
          overviewReport.charts.file = fileChart;

          writeOverview(overview, overviewReport, {
            title: options.title,
            flags: flags
          }, options.static ? staticOverviewTemplate : overviewTemplate);
